...
function LogoJJ({ onClick }) {
  ...
  <div className="flex flex-col dark:flex-col-reverse">
    ...
    <img onClick={onClick} className="dark:visible invisible h-0 w-0 m-0 p-0 dark:mt-5 relative dark:w-44 dark:h-[2.3rem] dark:sm:w-[12rem] dark:sm:h-[2.5rem] dark:2xl:mr-24" src={AWSUrl.URL + "logo-juan-jaramillo.png"} alt={t("Menu.title", { framework: "React" })} title={t("Menu.title", { framework: "React" })} />
  </div>
  );
}
